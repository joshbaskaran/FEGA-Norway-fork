...
public class Crypt4GHUtils {
   ...
   */
  public Header setRecipient(
      byte[] serializedHeader, PrivateKey privateKeyForDecryption, PublicKey newRecipientPublicKey)
    ...
    try (ByteArrayInputStream headerInputStream = new ByteArrayInputStream(serializedHeader);
        Crypt4GHInputStream crypt4GHInputStream =
            new Crypt4GHInputStream(headerInputStream, privateKeyForDecryption)) {
      Header header = crypt4GHInputStream.getHeader();
      List<HeaderPacket> headerPacketsWithNewRecipient =
          getHeaderPacketsWithNewRecipient(header, privateKeyForDecryption, newRecipientPublicKey);
