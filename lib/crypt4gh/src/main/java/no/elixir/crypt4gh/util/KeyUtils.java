...
public class KeyUtils {
  ...
  public byte[] encodeKey(Key key) throws GeneralSecurityException {
    ...
    throw new GeneralSecurityException(
        "Expected either XECPublicKey or XECPrivateKey, but got: " + key.getClass());
  }
  ...
  public byte[] getU(PublicKey publicKey) throws GeneralSecurityException {
    ...
    ArrayUtils.reverse(
        u); // conversion from BigInteger to byte[] reverses array, thus reversing it back
    return Arrays.copyOf(
        u, 32); // if array ends with zeroes, they will be omitted during conversion, thus appending
    // them back
  }
   ...
   * @return X25519 private key.
   * @throws GeneralSecurityException In case the X25519 private key can't be constructed from the
   *     given scalar.
   */
  public PrivateKey constructPrivateKey(byte[] scalar) throws GeneralSecurityException {
    KeyFactory keyFactory = KeyFactory.getInstance(X25519);
    return keyFactory.generatePrivate(
        new XECPrivateKeySpec(new NamedParameterSpec(X25519), scalar));
  }
   ...
   * @throws GeneralSecurityException In case the X25519 public key can't be constructed from the
   *     given U.
   */
  public PublicKey constructPublicKey(byte[] u) throws GeneralSecurityException {
    ...
    u = u.clone();
    ArrayUtils.reverse(
        u); // conversion from byte[] to BigInteger will reverse array, thus reversing it here in
    // the first place
  ...
  public PublicKey derivePublicKey(PrivateKey privateKey) throws GeneralSecurityException {
    ...
    keyPairGenerator.initialize(
        new NamedParameterSpec(X25519), new StaticSecureRandom(getScalar(privateKey)));
    return keyPairGenerator.generateKeyPair().getPublic();
  }

  /**
   * Generates Diffie Hellman shared key from sender's X25519 private and recipient's X25519 public
   * keys.
   *
   ...
   */
  public byte[] generateDiffieHellmanSharedKey(PrivateKey privateKey, PublicKey publicKey)
  ...
  /**
   * Generates writer's shared key based on sender's X25519 private and recipient's X25519 public
   * keys.
   *
   ...
   */
  public SecretKey generateWriterSharedKey(PrivateKey writerPrivateKey, PublicKey readerPublicKey)
    ...
    byte[] digest =
        MessageDigest.getInstance(Blake2b.BLAKE2_B_512)
            .digest(
                ArrayUtils.addAll(
                    ArrayUtils.addAll(diffieHellmanKey, encodeKey(readerPublicKey)),
                    encodeKey(writerPublicKey)));
    return new SecretKeySpec(Arrays.copyOfRange(digest, 0, 32), CHA_CHA_20);
  }

  /**
   * Generates reader's shared key based on recipient's X25519 private and sender's X25519 public
   * keys.
   *
   ...
   * @throws GeneralSecurityException In case there's a problem in generating keys.
   */
  public SecretKey generateReaderSharedKey(PrivateKey readerPrivateKey, PublicKey writerPublicKey)
      throws GeneralSecurityException {
   ...
   * @param password Optional password (if private key is password-protected). Can be null for
   *     unencrypted key.
   * @return Private key.
   * @throws IOException If the file can't be read.
   * @throws GeneralSecurityException If the key can't be constructed from the given file.
   */
  public PrivateKey readPrivateKey(File keyFile, char[] password)
   ...
   * @return Private key.
   * @throws GeneralSecurityException If the key can't be constructed from the given content.
   * @throws IllegalArgumentException If the key is password-protected, but the password was <code>
   *     null</code>.
   */
  public PrivateKey readPrivateKey(String keyMaterial, char[] password)
   ...
   *     null</code>.
   */
  public PrivateKey readCrypt4GHPrivateKey(byte[] keyMaterial, char[] password)
      throws GeneralSecurityException, IllegalArgumentException {
    ...
    if (kdf != KDF.NONE) {
      if (password == null) {
        throw new IllegalArgumentException(
            "Private key is password-protected, need a password for decryption");
      }
    ...
    if (kdf == KDF.NONE) {
      if (cipher != Cipher.NONE) {
        throw new GeneralSecurityException(
            "Invalid private key: KDF is 'none', but cipher is not 'none");
      }
  ...
  public void writeCrypt4GHKey(Writer writer, Key key, char[] password)
    ...
    if (isPublic) {
      ...
      SecretKeySpec derivedKey =
          new SecretKeySpec(KDF.SCRYPT.derive(0, password, salt), CHA_CHA_20);
      Arrays.fill(password, (char) 0);
